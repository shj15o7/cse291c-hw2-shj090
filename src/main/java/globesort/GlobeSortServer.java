...
public class GlobeSortServer {
    ...
    static class GlobeSortImpl extends GlobeSortGrpc.GlobeSortImplBase {
        ...
        @Override
        public void sortIntegers(IntArray req, final StreamObserver<ServResponse> responseObserver) {
            ...
            ServResponse response = ServResponse.newBuilder().setResult(sortResponse).setSortTime(sortTime).build();
            responseObserver.onNext(response);
            responseObserver.onCompleted();
        }
    }
